...
The architecture manages 7 API keys across 2 1Password vaults (OpenAI, Google, Replicate, Apify, RemoveBG, YouTube, and GitHub) using UUID-based `op://` references for stability. It includes shell aliases for one-command PAI session launches, a conditional `gh` CLI integration that eliminates per-command biometric prompts, and a clear security model that explicitly states what it does and doesn't protect against.
